...
if test $# -eq 0; then
  ...
  /opt/witness_node --data-dir /opt/coin --track-account "\"$track\"" &
  ...
  # wallet
  exec /opt/cli_wallet --server-rpc-endpoint ws://127.0.0.1:8090 --rpc-http-endpoint 0.0.0.0:8091 --wallet-file /opt/coin/wallet.json
else
  exec $@
fi
